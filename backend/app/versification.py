"""Verse counts per book and chapter, for the standard Protestant canon.

Generated once from api.bible's NIV, whose versification matches the ESV's.
Used to size a requested range without fetching it — needed to enforce
Crossway's limit on how much ESV text may be shown at once.
"""
...
VERSE_COUNTS: dict[str, list[int]] = {
    ...
    "JOB": [
        ...
        29, 34, 30, 17, 25, 6, 14, 23, 28, 25, 31, 40, 22, 33, 37, 16, 33, 24, 41, 30,
        24, 34, 17
    ],
    "PSA": [
        6, 12, 8, 8, 12, 10, 17, 9, 20, 18, 7, 8, 6, 7, 5, 11, 15, 50, 14, 9, 13, 31,
        6, 10, 22, 12, 14, 9, 11, 12, 24, 11, 22, 22, 28, 12, 40, 22, 13, 17, 13, 11,
        5, 26, 17, 11, 9, 14, 20, 23, 19, 9, 6, 7, 23, 13, 11, 11, 17, 12, 8, 12, 11,
        10, 13, 20, 7, 35, 36, 5, 24, 20, 28, 23, 10, 12, 20, 72, 13, 19, 16, 8, 18,
        12, 13, 17, 7, 18, 52, 17, 16, 15, 5, 23, 11, 13, 12, 9, 9, 5, 8, 28, 22, 35,
        45, 48, 43, 13, 31, 7, 10, 10, 9, 8, 18, 19, 2, 29, 176, 7, 8, 9, 4, 8, 5, 6,
        5, 6, 8, 8, 3, 18, 3, 3, 21, 26, 9, 8, 24, 13, 10, 7, 12, 15, 21, 10, 20, 14,
        9, 6
    ],
    "PRO": [
    ...
    "EZK": [
        ...
        32, 31, 49, 27, 17, 21, 36, 26, 21, 26, 18, 32, 33, 31, 15, 38, 28, 23, 29,
...
def chapter_count(book_id: str) -> int:
    """Number of chapters in a book, or 0 if it is not in the canon."""
    return len(VERSE_COUNTS.get(book_id, ()))


def book_verse_total(book_id: str) -> int:
    return sum(VERSE_COUNTS.get(book_id, ()))
...
def verses_in_range(
    ...
    """How many verses a chapter-spanning range covers."""
    ...
    for chapter in range(start_chapter, end_chapter + 1):
        ...
        last = end_verse if chapter == end_chapter else counts[chapter - 1]
        total += max(0, min(last, counts[chapter - 1]) - first + 1)
    return total
